% ============================================================================
% paper9_v7_draft_ncs.tex --- Nature Computational Science variant
% ============================================================================
% Forked from paper9_v7_draft.tex 2026-05-30 in response to a target-journal
% reassessment: the manuscript's core contribution is a methodological
% diagnose-then-fix of MSE-trained learned planning models in high-branching
% discrete domains, which is a stronger fit for NCS than for Nature
% Sustainability. This variant retains the application content but reframes
% the contribution as a methods paper, addressing the simulated NCS reviewer
% concerns documented at notes/world_model_comparison.md and the major-revision
% checklist (M1-M5):
%
%   M1: "World model" terminology clarified; primary term changed to
%       "learned transition model with reward head" except where invoking
%       the broader model-based RL literature explicitly.
%   M2: §3.5 promoted to a self-contained subsection on the aggregate-fidelity
%       vs per-action-ranking decomposition, with an analytic example
%       (constant-predictor lower bound) and explicit evaluation-methodology
%       implications.
%   M3: Discussion §"Beyond this benchmark" rewritten to make the connection
%       to RLHF reward modelling, MuZero policy/value networks, and AlphaZero
%       discriminative planning explicit, instead of generic future-work hedging.
%   M4: Abstract claims tightened: "second county within the same regulatory
%       regime", "in-house DRL baselines" (not "RL baselines tout court").
%   M5: §3 Table 1 (lambda ablation rank acc) gets an explicit footnote on
%       absolute-numbers reproducibility status (qualitative trend reproduces
%       on independently-generated pairwise; absolute numbers depend on the
%       evaluation pairwise distribution; trained checkpoints + script
%       released for verification).
%
% Numbers verified against the original v7 draft and macOS reproduction
% artefacts (paper/repro_artifacts/macos_2026-05-29/) committed to the
% public repo; no fabricated data, no claim weakened that the underlying
...
\title{When learned-model planning beats classical optimisation: simulator cost and combinatorial cost-interactions as the binding factors in high-branching geospatial decisions}
...
\begin{abstract}
% ============================================================================
Model-based reinforcement learning is widely viewed as the natural fit for combinatorial planning problems with structured state and a known cost function, but applications to high-branching discrete action spaces have repeatedly been outperformed by classical operations-research methods (greedy heuristics, simulated annealing, mixed-integer programming) without a clear diagnosis. Here we identify the conditions under which a learned-surrogate planner provides genuine, irreplaceable value over those baselines, by sweeping simulator step cost and reward-landscape structure across ten geospatial planning configurations. Two binding factors emerge. First, when each environment step is computationally expensive ($\geq 30$\,ms in our measurements), oracle-based search budgets collapse: simulated annealing with a fixed wall-clock budget completes only $\sim 100$ iterations on the farmland-consolidation simulator (17.9\,ms/step) versus $\sim 9{,}000$ on the cheap-step restoration simulator (5\,$\mu$s/step), and at the farmland step cost the learned-surrogate model-predictive control (MPC) pipeline produces episode rewards 9.6$\times$ those of simulated annealing under matched wall budgets. Second, when reward exhibits combinatorial cost interactions (one action's effective cost depends on the already-selected set), the surrogate's batched action scoring exposes the joint structure that iterative oracle search cannot afford to enumerate, and MPC dominates classical methods even at zero step cost. Within the regime where these factors bind, we additionally document an internal failure mode of mean-squared-error training: aggregate prediction fidelity (cosine similarity 0.998) and within-state action ranking accuracy (51.6\%) are not constrained against each other, and the latter is what model-predictive top-$K$ selection actually consumes; an auxiliary pairwise large-margin loss on the existing reward head raises ranking accuracy to 85.5\% and converts MPC from the worst variant tested to the strongest. The end-to-end pipeline reproduces the headline farmland result on real cadastral data from two southwest-China counties (128{,}891 parcels), validates on seven parameter-controlled synthetic landscapes, and is released alongside a deployable ArcGIS Pro toolbox plus a non-commercial CLI workflow.
\end{abstract}
...
\paragraph{Why a learned surrogate when the simulator is known.} A natural objection is that the planning environments studied here are rule-based GIS simulators whose state and reward are fully known, so why not directly optimise on the simulator? The answer is computational throughput, not statistical fidelity. The MPC variant of our pipeline scores $K{=}50$ candidate actions in parallel via a single ONNX batch call ($\sim 25$\,ms on a desktop CPU); an equivalent true-environment evaluation would have to call \texttt{env.step} 50 times sequentially because the simulator has incremental geometric state that does not vectorise (\S\ref{sec:methods-trueenv}; we measure this directly: at matched $H{=}5$, $K{=}50$, true-env MPC is $4\times$ slower per episode and forced to subsample stage-1 actions, producing strictly worse plans on all three objectives). A learned surrogate is therefore best read here as a \emph{batch-friendly action scorer} that grants the planner depth and breadth---not as a statistical model of an unknown stochastic environment. This re-frames the methodological question: a learned model does not need to be calibrated; it needs to rank.

\paragraph{Terminology and scope.} We use \emph{learned planning model} (or, when invoking the broader literature, the operational model-based-RL sense of \citealp{chua2018deep,janner2019trust}: a learned $\hat{p}(s_{t+1}, r_t \mid s_t, a_t)$ used for MPC lookahead or dream-trained policy training) to refer to the architecture studied here. We deliberately avoid the contemporary "world model" terminology of the Dreamer family \citep{hafner2025dreamerv3,hansen2024tdmpc2}, which encodes raw observations into a recurrent latent state, and of generative video models \citep[e.g.][]{ha2018world}: our setting is fully observed and episodic on pre-extracted cadastral block features, so neither representation learning nor partial-observability machinery applies. Our model is a feed-forward deterministic next-state predictor with a discriminative reward head---closer in spirit to MuZero's value/policy network \citep{schrittwieser2020muzero}, AlphaZero-style move evaluators, and RLHF-style learned reward models \citep{christiano2017deep,ouyang2022training} than to the generative latent-dynamics world models that dominate current usage of the term.

Here we identify the binding factors that decide when learned-surrogate model-predictive control (MPC) provides genuine, irreplaceable value over classical operations-research methods (greedy heuristics, simulated annealing, NSGA-II, mixed-integer programming). On the cadastral-consolidation simulator the learned-surrogate pipeline reaches a slope reduction of $-1.289 \pm 0.079\%$ (Bishan), which is 1.6$\times$ the magnitude obtained by an in-house centralised-PPO baseline trained at $8$--$12$ GPU-hours per seed; under matched 180\,s wall-clock budgets, simulated annealing on the same simulator reaches an episode reward of only $9.0$ versus the contrastive MPC's $86.7$, a 9.6$\times$ gap. By contrast, on cheap-step problems (an abandoned-mine-restoration case at 5\,$\mu$s/step), simulated annealing matches or exceeds learned-surrogate MPC at equal wall-clock budgets. The factor that toggles between these two regimes is the per-step simulator cost: simulated annealing's iteration count under a fixed wall budget collapses linearly with step cost (about $9{,}000$ iterations at 5\,$\mu$s/step, $\sim 100$ at 18\,ms/step, $\sim 6$ at 100\,ms/step), while the learned surrogate scores all $K{=}50$ candidate actions in a single batched ONNX call ($\sim 25$\,ms regardless of underlying simulator cost) and therefore preserves search depth and breadth that the iterative oracle cannot afford (\S\ref{sec:simcost}). A second, independent factor is the structure of the reward: when each action's effective cost depends on the already-selected set (combinatorial cost interactions, of which farmland's \textit{baimu fang} aggregation is a representative example), the learned surrogate dominates classical methods even at zero step cost (\S\ref{sec:cross-domain}). Within the regime where these factors bind, we additionally identify and fix a hidden failure mode of mean-squared-error training: aggregate prediction fidelity (cosine similarity $0.998$) and within-state action ranking accuracy ($51.6\%$, statistically indistinguishable from random) are not constrained against each other, and the latter is what model-predictive top-$K$ selection actually consumes (\S\ref{sec:fidelity-vs-ranking}); an auxiliary pairwise large-margin loss \citep{burges2005learning,joachims2002optimizing,herbrich2000large} on the existing reward head raises ranking accuracy to $85.5\%$ and converts MPC from the worst variant tested to the strongest. The diagnostic generalises across geospatial planning regimes; the intervention is conditional on the ranking-failure regime ($\sigma_a / \sigma_s \lesssim 1$) that the diagnostic identifies. We release the trained ensembles, an open synthetic benchmark, the public-data restoration cases, and a deployable ArcGIS Pro toolbox alongside a non-commercial CLI workflow.
...
\subsection*{Aggregate fidelity is not constrained against within-state action ranking by MSE training}\label{sec:fidelity-vs-ranking}

The decoupling we observe empirically is not a quirk of our specific architecture, dataset, or optimiser; it is a property of the loss. Let $\hat{r}_\theta(s, a)$ be the reward head of a learned model trained on $(s, a, r)$ pairs sampled from a distribution $\mu(s, a)$, and let
\begin{equation}
\mathcal{L}_{\text{MSE}}(\theta) \;=\; \mathbb{E}_{(s,a)\sim\mu}\big[(\hat{r}_\theta(s,a) - r(s,a))^2\big]
\end{equation}
be the standard objective. Two distinct quantities can be evaluated on this learned model: \emph{aggregate fidelity}, defined here as the population MSE itself, and \emph{within-state ranking accuracy}
\begin{equation}
...
The two quantities are not constrained against each other under MSE training: minimising aggregate population MSE places no penalty on assigning two actions the same predicted reward when their true rewards differ. A trivial pathological example makes this concrete. Consider a setting in which, at every state $s$, the action-conditioned reward distribution is symmetric around a state-dependent mean $\bar{r}(s)$ with state-marginal variance $\sigma_s^2$ and within-state action variance $\sigma_a^2(s)$. The learned predictor $\hat{r}_\theta^{\star}(s,a) = \bar{r}(s)$, which assigns the conditional mean to every action regardless of its identity, achieves population MSE equal to $\mathbb{E}_s[\sigma_a^2(s)]$ (the irreducible state-conditional variance) and within-state ranking accuracy equal to $1/2$ on every pair with non-zero true reward difference, identical to random guessing. A learned model can therefore minimise MSE within a constant of the Bayes optimum while remaining indistinguishable from random on the very ranking decisions that drive planning. Our $51.6\%$ rank accuracy at cosine similarity $0.998$ is empirically close to this regime: the predicted reward standard deviation across the action menu (Table~\ref{tab:contrastive-disc}) is compressed by an order of magnitude relative to the true within-state variance, exactly as the conditional-mean predictor would behave. We emphasise that this is a counterexample showing MSE \emph{does not guarantee} ranking fidelity in regimes where $\sigma_a^2(s) \ll \sigma_s^2$, not a statement of statistical independence: in regimes where $\sigma_a^2(s)$ is large relative to $\sigma_s^2$, MSE training will in practice acquire much of the ranking signal as a by-product (\S\ref{sec:cross-domain} demonstrates this empirically).

The implication for evaluation methodology is direct. Standard model-based reinforcement-learning benchmarks---DM Control, Atari-100k, the continuous-control suites underpinning Dreamer-family results \citep{hafner2025dreamerv3,hansen2024tdmpc2}, the low-branching board games studied by MuZero \citep{schrittwieser2020muzero}---are dominated by settings in which the action menu at any given state contains few alternatives or is parameterised continuously. In both regimes, ranking errors translate into small policy-level differences and are absorbed by the planner without surfacing as failure. The decomposition above predicts that learned models passing aggregate-fidelity tests in those regimes can fail catastrophically when transferred to high-branching discrete domains \emph{with low within-state action variance}---and our four observed failure modes (dream-policy collapse, MPC ceiling at $-0.21\%$, greedy-continuation reversal, retrained-baseline persistence) are exactly that pattern. Whether this prediction holds beyond cadastral planning---in job-shop scheduling, virtual-screening prioritisation, urban renewal block selection, transportation-network reconfiguration---remains an open question that the diagnostic of this paper makes testable: report ranking accuracy, top-$K$ regret, and within-state action variance on the action menus the planner actually faces, before deciding whether the contrastive intervention is needed.
...
\paragraph{Comparison against classical OR baselines.} For completeness we also evaluated a panel of operations-research baselines on the bishan CountyLevelEnv at a 180\,s wall budget (matched to the contrastive-MPC pipeline's wall time per episode). Simulated annealing over a permutation of block selections reached a cumulative episode reward of $9.04$ after 100 iterations; random-restart over 98 episodes reached $30.85$. Contrastive MPC under the same wall budget reached $86.74$ — a $9.6\times$ gap over simulated annealing and a $2.8\times$ gap over random restart. The mechanism behind this gap is the simulator's per-step cost (17.9\,ms/step on bishan) combined with combinatorial cost interactions in the reward function; we develop the full crossover analysis in \S\ref{sec:simcost}. The headline conclusion is that, at the natural step cost of the farmland simulator, classical optimisation methods are budget-starved within any wall-clock budget a county planner would be willing to wait for, while the learned-surrogate pipeline is not.
...
% ============================================================================
\section{Cross-domain verification: the diagnostic generalises, the intervention is conditional}\label{sec:cross-domain}
% ============================================================================

The synthetic stress tests above vary geometry and fragmentation but keep the underlying decision semantics (paired farmland--forest swaps under township-level swap budgets, area-weighted slope and contiguity rewards) identical to Sections~\ref{sec:counties}. To probe the boundary of the methodological contribution of \S\ref{sec:fidelity-vs-ranking}--\ref{sec:fix}, we apply the unmodified pipeline to a structurally different geospatial planning problem: ecological-restoration prioritisation for abandoned mine lands. The decision unit is a planning cell (not a cadastral block), the action is a one-time selection of a unit for restoration (not a reversible paired swap), and the reward is a linear combination of risk reduction, water-quality protection, spatial connectivity, and a cost penalty (not the farmland multi-objective reward of Equation~\ref{eq:reward}). We test on two cases: a real public-data instance from Buchanan County, Virginia, USA ($n_{\text{units}}=562$ planning cells at 2\,km resolution; $1{,}249$ abandoned-mine point records from the OSMRE e-AMLIS public inventory; $3{,}157$ NHD flowlines for water-priority scoring; USGS 3DEP slope at 30\,m resolution), and a deterministic synthetic instance with $n_{\text{units}}=420$ on a comparable scale. The pipeline (Tools 1--4) is structurally unchanged: only the per-unit feature vector, the multi-objective reward weights, and the action-mask semantics are swapped (\S\ref{sec:methods-restoration}).

\paragraph{Framing.} We use this case as a regime diagnostic, not as a generalisation claim for the contrastive intervention. The cross-county evidence in \S\ref{sec:counties} already establishes that the intervention solves the farmland regime; the question here is whether the underlying \emph{decoupling} of \S\ref{sec:fidelity-vs-ranking} also predicts \emph{where the intervention is unnecessary}. To make this falsifiable we expand the baseline panel beyond the farmland comparison and explicitly include classical operations-research methods (greedy by attribute, simulated annealing, NSGA-II, and a CBC-MILP weighted-sum formulation) alongside random-uniform, MSE-only learned MPC and contrastive learned MPC. We additionally fix the unfair design choice in earlier drafts where MSE-only was a single ensemble while contrastive was 5 seeds: both training objectives now use 5 independently trained ensembles per case.

\paragraph{Result 1: classical OR baselines reach near-optimum, not just MPC variants.} Table~\ref{tab:restoration-baselines} reports total cumulative episode reward across seven methods on each case. On Buchanan, simple greedy-by-priority selection ($+228.76$) is statistically indistinguishable from CBC-MILP ($+229.43$) and from the learned-surrogate MPC variants (MSE-only $+229.20 \pm 0.14$, contrastive $+229.39 \pm 0.03$ across 5 seeds). NSGA-II ($+175.45$) under-performs because it is optimising a separate Pareto frontier across four objective components rather than the weighted-sum scalar reward. On synthetic, a similar pattern holds: simulated annealing ($+8{,}737$) and greedy ($+8{,}591$) match or exceed the learned-MPC variants ($+8{,}173 \pm 66$ MSE-only; $+8{,}128 \pm 63$ contrastive). The order-of-magnitude lift is over the random baseline, not over OR baselines: contrastive MPC more than doubles random's reward on Buchanan and improves it by $+32\%$ on synthetic, but the gap to a well-engineered OR baseline is within seed-level noise.
...
\paragraph{Result 2: MSE-only and contrastive surrogates are equivalent on the planner-relevant ranking metrics.} Beyond the pairwise accuracy used in \S\ref{sec:rank}, we evaluate planner-aligned ranking quality with NDCG@$K$, top-$K$ regret, and Spearman/Kendall rank correlation across the action menu, computed on each case's held-out pairwise dataset. On Buchanan, MSE-only and contrastive ensembles are essentially identical on every metric we report (Table~\ref{tab:restoration-ranking}): pairwise accuracy is $\geq 0.95$ in both, NDCG@1 reaches $0.999$ in both, top-1 regret is $< 0.003$ in both, Spearman correlation is $\sim 0.95$ in both. The same pattern holds on synthetic. By contrast, on Bishan the MSE-only model has pairwise accuracy $0.516$ (random-chance) and the contrastive model has $0.855$. The diagnostic of \S\ref{sec:fidelity-vs-ranking} explains both regimes: in farmland the within-state action variance is small relative to between-state variance ($\sigma_a = 0.811$ on Bishan, computed from the held-out pairwise dataset; the conditional-mean predictor is therefore an MSE-near-optimal solution that gives 50\% rank accuracy by construction), while in restoration the per-action reward is largely a per-unit attribute lookup, so $\sigma_a$ is large relative to $\sigma_s$ ($\sigma_a = 0.47$ on Buchanan, $34.4$ on synthetic) and MSE training acquires the ranking signal as a by-product.

\begin{table}[htbp]
\centering
\small
\caption{Planner-relevant ranking metrics on the restoration cases (3-member ensemble at the indicated $\lambda_{\text{rank}}$, evaluated on the held-out pairwise dataset). Buchanan-MSE and Buchanan-contrastive are statistically indistinguishable on every metric; same for synthetic. By contrast, on Bishan the MSE-only ensemble has pairwise accuracy $0.516$ (Table~\ref{tab:contrastive-disc}). The contrastive intervention is therefore beneficial only when ranking failure is actually present.}
\label{tab:restoration-ranking}
\begin{tabular}{l@{\hspace{0.5em}}cccc@{\hspace{0.5em}}cccc}
\toprule
 & \multicolumn{4}{c}{Buchanan VA} & \multicolumn{4}{c}{Synthetic} \\
\cmidrule(lr){2-5}\cmidrule(lr){6-9}
Metric & MSE-only & contrastive & $\Delta$ & & MSE-only & contrastive & $\Delta$ & \\
\midrule
pairwise accuracy            & 0.951 & 0.951 & $+0.000$ & & 0.971 & 0.974 & $+0.003$ & \\
Spearman rank correlation    & 0.951 & 0.949 & $-0.002$ & & 0.973 & 0.977 & $+0.004$ & \\
Kendall $\tau$               & 0.951 & 0.949 & $-0.002$ & & 0.973 & 0.977 & $+0.004$ & \\
NDCG@1                       & 0.999 & 0.999 & $+0.000$ & & 0.999 & 0.999 & $+0.000$ & \\
NDCG@5                       & 0.999 & 0.999 & $+0.000$ & & 1.000 & 1.000 & $+0.000$ & \\
NDCG@10                      & 0.999 & 0.999 & $+0.000$ & & 1.000 & 1.000 & $+0.000$ & \\
Precision@10                 & 0.963 & 0.954 & $-0.009$ & & 0.953 & 0.969 & $+0.016$ & \\
Top-1 regret                 & 0.002 & 0.003 & $+0.001$ & & 0.001 & 0.001 & $+0.000$ & \\
Top-10 regret                & 0.002 & 0.003 & $+0.001$ & & 0.005 & 0.004 & $-0.001$ & \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Result 3: the planner exposes a clean Pareto front across reward weights.} The four-objective reward of the restoration env (risk reduction, water/habitat gain, connectivity, cost penalty) makes the trade-off between objectives explicit and configurable. Sweeping seven preset weight configurations (risk-, water-, connectivity-, cost-dominant; balanced; default; default-without-cost), Figure~\ref{fig:restoration-pareto}d--e shows the resulting Pareto frontier on the cumulative-component plane. The shape is robust across cases: a clear risk--water trade-off on the front, a connectivity outlier when explicitly weighted, and the default and balanced configurations sitting at the elbow. This illustrates why a planning office wanting to explore policy preferences would choose a learned-surrogate pipeline over a single OR optimisation: each Pareto point is one $\sim 28$\,s plan call (or $<1$\,s for the heuristic baselines), so a full sweep over weight configurations is feasible in a working day, where re-running NSGA-II at production scale would be substantially more expensive in the discrete-spatial regime.
...
\paragraph{Synthesis.} The restoration cases together establish three things. First, the diagnostic of \S\ref{sec:fidelity-vs-ranking} (small within-state $\sigma_a$ implies MSE-induced ranking collapse, large $\sigma_a$ does not) is empirically falsifiable and is correctly falsified here. Second, the contrastive intervention is conditional on the ranking-failure regime: it is necessary when the diagnostic predicts failure (farmland) and inert when the diagnostic predicts the converse (restoration). Third, the learned-surrogate planning pipeline has no monopoly on this kind of optimisation when classical OR methods scale---greedy heuristics and MILP reach near-optimum on the restoration cases as fast or faster than learned MPC. The methodological contribution of this paper is therefore the diagnostic itself plus the regime where it is binding, not a universal claim about model-based planning.
...
The cross-domain analysis of \S\ref{sec:cross-domain} closed one question (when does the contrastive ranking loss help?) but raised another. On the restoration cases, classical OR baselines (greedy by attribute, simulated annealing, NSGA-II, CBC-MILP) reached near-optimum statistically indistinguishable from the learned-MPC variants under the wall-clock budgets reported in Table~\ref{tab:restoration-baselines}, while on farmland the same algorithms underperformed by an order of magnitude. The natural question is what changed between the two regimes; the natural answer is the cost of one simulator call.

We test this directly. We instrument the restoration env with a fixed sleep injected on every \texttt{env.step} and re-run simulated annealing and learned-MPC at matched 30-second wall budgets across delays of \{0, 1, 10, 50, 100\}\,ms per step, on each of the ten (case~$\times$~reward-profile) cells. Figure~\ref{fig:simcost} reports the per-cell episode-reward gap (MPC minus SA, normalised by mean reward magnitude) and the SA iteration count actually completed in the budget.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{sim_cost_crossover.pdf}
\caption{Simulator step cost is the dominant predictor of when learned-surrogate MPC beats simulated annealing under matched wall-clock budgets. (a)~Reward gap (MPC minus SA, normalised by per-cell mean reward) versus injected step delay (log scale) on each of the ten (case~$\times$~reward-profile) cells. The shaded green band marks the empirical MPC-advantage zone (delay $\geq 30$\,ms). The actual farmland Bishan datapoint (gold star), measured at 17.9\,ms/step natural step cost under a 180\,s wall budget, sits at $+89.6\%$. (b)~SA iteration count completed in the same wall budget, on a log-y axis: the count collapses linearly with step cost from $\sim 9{,}000$ at 5\,$\mu$s/step to $\sim 6$ at 100\,ms/step. Below $\sim 100$ iterations SA cannot converge, which is the mechanism by which step cost translates into the MPC advantage. The synthetic/scale\_economy line is the exception: MPC dominates at every step delay because the combinatorial cost-interaction structure of that profile is what learned-surrogate batched scoring exposes naturally.}
\label{fig:simcost}
\end{figure}

The pattern across the cells is consistent with two independent factors.

\paragraph{Factor 1: simulator step cost.} On nine of the ten cells where reward is dominated by per-unit attribute lookups (default, connectivity\_dominant, watershed, delayed; both Buchanan and synthetic), the gap MPC minus SA is monotonic in step delay. Buchanan/delayed crosses zero at $\sim 30$\,ms (Figure~\ref{fig:simcost}a, dark-blue downward triangle), reaching $+31.2\%$ at 100\,ms. The mechanism is direct (Figure~\ref{fig:simcost}b): SA's iteration count under a fixed wall budget falls from $\sim 9{,}000$ at 5\,$\mu$s/step to $\sim 6$ at 100\,ms/step, while learned-MPC's per-step cost (one batched ONNX forward pass over $K{=}50$ candidates, $\sim 25$\,ms) is decoupled from \texttt{env.step}. Below the rough threshold of $\sim 100$ iterations the SA neighbourhood walk cannot converge on a 50-unit permutation problem, and the learned-surrogate planner overtakes by structural advantage rather than statistical accuracy.

\paragraph{Factor 2: combinatorial cost interactions.} The exception in Figure~\ref{fig:simcost}a is the synthetic/scale\_economy line (red diamond), where MPC dominates SA by $+542$ to $+931$ across every step delay including 5\,$\mu$s. Under the scale\_economy reward profile, an action's effective cost depends on the already-selected set (a regional restoration scale economy: cost discounts on units with already-selected 2-hop neighbours), so the optimisation surface contains joint interactions that are not visible to a permutation-search procedure that ranks units independently. The learned surrogate's batched scoring of all candidate actions at the current state internalises these interactions naturally because it conditions on a single global state vector capturing the cumulative selection. This advantage is independent of step cost.

\paragraph{Empirical confirmation on the actual farmland simulator.} The two factors compose in the farmland-consolidation environment. We measured the natural step cost of the bishan CountyLevelEnv to be $17.9 \pm 0.3$\,ms (five-step probe, repeated three times) — squarely in the regime where SA's iteration count at a $180$\,s budget is $\sim 100$, near the convergence threshold. Reward additionally exhibits combinatorial cost interactions: \textit{baimu fang} count is the union-find aggregation over per-block selections, contiguity is the area-weighted adjacency average, and slope is the area-weighted geometric integral over per-parcel slopes that updates with every paired swap. Under matched 180\,s wall budgets, simulated annealing on Bishan reaches an episode reward of $9.04$ ($n_{\text{iter}}=100$), random restart reaches $30.85$ ($n_{\text{eps}}=98$), and contrastive MPC reaches $86.74$ — a $9.6\times$ gap over SA and a $2.8\times$ gap over random restart. This is the empirical reason the contrastive-MPC pipeline dominates farmland but not restoration~default: farmland satisfies both factors simultaneously while restoration~default satisfies neither, and the cells in between (restoration/scale\_economy and restoration/delayed at high step delay) match exactly the patterns the two factors predict.

\paragraph{Implication for problem selection.} Combined, the analysis gives a falsifiable test for whether learned-surrogate MPC will outperform classical OR on a new application: report the simulator step cost (in ms/step) and the cumulative-cost or cumulative-reward dependence of the reward function on the already-selected action set. When step cost exceeds $\sim 30$\,ms or when reward exhibits non-trivial combinatorial cost interactions (or both), MPC is in the advantage regime; when neither condition holds, well-tuned SA or MILP will reach near-optimum at lower wall-clock cost. We expect this criterion to extend beyond the farmland-cadastral domain to applications where high-fidelity physics simulators dominate the per-step cost (hydrologic-model-driven watershed restoration, multi-stage chemical reaction prioritisation, integrated power-grid maintenance scheduling, supply-chain disruption optimisation under economic equilibrium models), all of which combine non-trivial step cost with combinatorial constraint structure.
...
The toolchain has been integration-tested on a clean ArcGIS Pro 3.6 environment with the standard \texttt{arcgispro-py3} Python distribution and the two non-default dependencies (\texttt{gymnasium}, \texttt{libpysal}) installable through a single \texttt{conda} command. To support users without an ArcGIS Pro license, the same end-to-end pipeline is also exposed through a non-commercial command-line interface (\texttt{farmland-mpc prepare/sample/train/plan}) that runs against any QGIS-compatible vector and raster stack via \texttt{geopandas}/\texttt{rasterio} and produces the same on-disk artefacts as the ArcGIS toolbox. The package is released under an MIT licence; the underlying synthetic benchmark (Section~\ref{sec:bench}) is released under CC-BY 4.0. Together, the synthetic benchmark, the deterministic generator, the trained Bishan ensemble, the released restoration cases (\S\ref{sec:cross-domain}), and the four-tool ArcGIS pipeline form a complete reproduction kit: a reviewer or future user with either an ArcGIS Pro license or a free QGIS-compatible Python environment can move from ``I have a county shapefile'' to ``I have an optimised consolidation plan'' without access to either GPU compute or the original sensitive cadastral data.
...
\paragraph{When does this matter — and when does it not.} The cross-domain analysis (\S\ref{sec:cross-domain}) and the simulator-cost crossover (\S\ref{sec:simcost}) together reframe the contribution as conditional. The contrastive ranking loss is a fix for the specific failure mode where mean-squared-error training collapses to a conditional-mean predictor, which happens when within-state action variance is small relative to between-state variance ($\sigma_a / \sigma_s \lesssim 1$); in regimes where this ratio is large, mean-squared-error training already learns to rank and the contrastive term is redundant. The learned-surrogate MPC pipeline as a whole, separately, beats classical operations-research baselines (greedy, simulated annealing, NSGA-II, mixed-integer programming) only in two regimes: when each simulator step is computationally expensive enough ($\geq 30$\,ms in our measurements) that oracle-based search budgets collapse below the convergence threshold under any wall-clock budget a planner would tolerate, or when reward exhibits combinatorial cost interactions that batched ensemble scoring exposes more efficiently than iterative neighbourhood search. Farmland satisfies both conditions ($17.9$\,ms/step, \textit{baimu fang} aggregation as cost interaction); restoration's default does not (5\,$\mu$s/step, attribute-lookup reward). Practitioners considering whether to deploy a learned-surrogate planner on a new domain should report these two quantities first; the literature's default of comparing only against PPO or DQN routinely answers a less informative question.

\paragraph{Beyond this benchmark.} The diagnostic and the simulator-cost crossover both have structural reach beyond farmland consolidation. Wherever a learned model is used to rank a large discrete menu of actions at a single state---job-shop scheduling, water-allocation gate selection, urban renewal block prioritisation, virtual-screening lead selection, transportation-network reconfiguration---the relevant quantity for downstream planning is within-state ranking accuracy, not aggregate reconstruction error. The model-based reinforcement-learning literature has mostly evaluated learned models on continuous-control benchmarks \citep{hafner2025dreamerv3,hansen2024tdmpc2,chua2018deep} or low-branching board games \citep{schrittwieser2020muzero}, where the action menu at any given state contains few alternatives or is parameterised continuously, and ranking errors translate into small policy-level differences absorbed by the planner without surfacing as failure (\S\ref{sec:fidelity-vs-ranking}). The fix we report---a discriminative ranking term applied to the existing reward head---sits in a methodological neighbourhood that is currently under-developed in the model-based-RL literature but well-developed elsewhere: MuZero's policy and value networks \citep{schrittwieser2020muzero} are discriminative scorers attached to a learned dynamics model and are explicitly trained to match search-derived rankings rather than to regress raw state-action returns; AlphaZero's move evaluator is the same idea in a low-branching setting; and the RLHF reward-modelling literature \citep{christiano2017deep,ouyang2022training,burges2010ranknet} is the canonical example of pairwise-preference training as the mechanism by which discriminative ranking signal enters a policy pipeline. The intervention reported here is best read as importing that discriminative-ranking idea from policy-side learned reward models to the model-side reward head used by MPC, in a regime---high-branching discrete spatial planning---where the generative-latent-dynamics framing common to Dreamer-style world models \citep{hafner2025dreamerv3,ha2018world} brings no benefit and the planning machinery only needs the model to rank, not to roll out. The auxiliary pairwise margin loss is cheap to add and, in our setting, sufficient. Whether per-action ranking is the binding constraint should be checked before more elaborate fixes are entertained. The simulator-cost criterion of \S\ref{sec:simcost} provides the parallel test on the algorithm-as-system level: report step cost and combinatorial structure before deploying.
...
\paragraph{What we did not do, and bounds on the claim.} Four constraints qualify the result. First, the cadastral evidence comes from two counties in southwestern China within the same regional regime; structurally different agricultural settings (flat large-parcel plains, non-Chinese tenure systems, water-system-dominated landscapes) remain untested as far as the contrastive intervention is concerned, although the cross-domain restoration cases (\S\ref{sec:cross-domain}) demonstrate that the underlying \emph{diagnostic} extends beyond the farmland regime even when the intervention itself does not. Second, the contrastive intervention's necessity has been characterised but not exhaustively swept: we identify a regime (small within-state action variance) where it helps and a regime (per-unit attribute lookup reward) where it does not, but a finer characterisation across the variance ratio $\sigma_a^2 / \sigma_s^2$ would let practitioners decide a priori whether to add the auxiliary loss. Third, the pairwise dataset relies on the simulator's ability to snapshot and restore the full state, available in our gridded-cadastral and unit-selection simulators but not in many physics-based environments; whether replay-buffer-based approximations suffice is an open question. Fourth, our $\lambda_{\text{rank}}$ and margin sweeps are coarse; finer hyperparameter resolution may yield further improvement but is not expected to reverse the qualitative ordering. The cross-domain analysis (\S\ref{sec:cross-domain}) also shows that classical OR baselines reach near-optimum in regimes where ranking failure is absent; in such regimes the learned-surrogate pipeline is justified by wall-clock and configurability for policy-preference exploration (Pareto-front sweeps), not by superior optimum quality.
...
The farmland cadastral data for Bishan District and Neijiang Dongxing District are derived from the Third National Land Survey of China and cannot be publicly redistributed under existing data-governance restrictions. All other reproduction materials are publicly available now (not gated on publication) at the project repository (\S Code Availability):

(a)~the full synthetic farmland benchmark of seven landscape presets with deterministic generators (Section~\ref{sec:bench}) under CC-BY 4.0;
(b)~aggregated block-level features (the 17-dimensional vectors for all $2{,}600$ Bishan and $3{,}711$ Neijiang blocks in the initial state) and the anonymised pairwise dataset $\mathcal{D}_{\text{pw}}$ for both farmland counties;
...
(e)~the complete restoration cases of \S\ref{sec:cross-domain}: \emph{Buchanan VA} (planning-unit attributes, adjacency, OSMRE e-AMLIS extracts, USGS NHD flowlines clipped to the county, USGS 3DEP slope, and Census TIGER county boundary, all already public per their respective licences) and the deterministic \emph{synthetic} mine-restoration generator with all seven weight-sweep Pareto configurations.

The farmland-specific items (b)--(d) are sufficient to reproduce transition-model training, contrastive training, and MPC evaluation pipelines end-to-end without access to the raw cadastral records; the restoration items in (e) are end-to-end reproducible from raw data because no licence restriction applies.
