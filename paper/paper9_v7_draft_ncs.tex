...
\title{Within-state ranking, not aggregate fidelity, limits learned-model planning in low-signal high-branching geospatial optimisation}
...
\begin{abstract}
% ============================================================================
Model-based reinforcement learning is widely viewed as the natural fit for combinatorial planning problems with structured state and a known cost function, but applications to high-branching discrete action spaces have repeatedly under-performed model-free baselines without a clear diagnosis. Here we identify a cause and a practical fix in a specific regime. Aggregate prediction fidelity (cosine similarity 0.998) and within-state action ranking accuracy (51.6\%, statistically indistinguishable from random) are not constrained against each other under mean-squared-error (MSE) training, and the latter is what model-predictive planning actually consumes. We document this decoupling in a county-scale farmland-consolidation benchmark with $\sim$2{,}600--3{,}700-way categorical action spaces, show that it accounts for four otherwise-puzzling failure modes of the learned planner, and demonstrate that augmenting the existing reward head with a pairwise large-margin loss raises ranking accuracy to 85.5\% without architectural change, converting model-predictive control from the worst variant to the strongest. The intervention is validated on real cadastral data from two counties of the same regulatory regime ($128{,}891$ parcels), seven parameter-controlled synthetic landscapes, and a five-layer verification stack including independent GIS recomputation from the optimised on-disk shapefile. Wall-clock for one planning scenario drops from 8--12 GPU-hours per random seed (in-house centralised-PPO and multi-agent baselines) to $\sim$7 minutes on a desktop CPU. To probe the boundary of the diagnostic, we also evaluate the pipeline on a non-farmland natural-resources case (abandoned-mine restoration prioritisation, both real US public data and a synthetic instance) and on classical OR baselines (greedy, simulated annealing, NSGA-II, and CBC-MILP). On the restoration cases the MSE-only and contrastive variants are within noise of each other (pairwise ranking accuracy $\geq 0.97$ for both, NDCG@10 $\geq 0.999$), and classical OR baselines also reach near-optimum---confirming that the ranking failure of the farmland regime is reward-landscape-specific (it requires small within-state action variance) rather than universal. The diagnostic generalises across geospatial planning regimes; the intervention is beneficial only when ranking failure is actually present. We release the trained ensembles, the open synthetic benchmark, and a deployable ArcGIS Pro toolbox alongside a reproducible non-commercial CLI workflow.
\end{abstract}
...
Here we show that the obstacle is not architectural but the training objective itself. We document a hidden ranking failure in standard mean-squared-error training of a county-scale learned planning model: despite high aggregate accuracy (cosine similarity 0.998), per-action reward predictions are essentially random (pairwise accuracy 51.6\%), and this accounts for four independent failure modes we observe across dream-trained policies and model-predictive control on real cadastral data from Bishan District, Chongqing. We then show that adding an auxiliary pairwise large-margin ranking loss---an existing learning-to-rank technique \citep{burges2005learning,joachims2002optimizing,herbrich2000large}, applied here to the model's reward head---closes this gap (pairwise accuracy 85.5\%) and converts model-predictive control from failure into the best-performing strategy. The intervention reproduces on a second county within the same regulatory regime (Neijiang Dongxing District, Sichuan) and on seven parameter-controlled synthetic stress-test landscapes spanning terrain, size, and fragmentation. To probe the boundary of the fix, we further test on a non-farmland natural-resources case (abandoned-mine restoration prioritisation, both real US public data and a synthetic instance), where the diagnostic of \S\ref{sec:fidelity-vs-ranking} correctly predicts that MSE-only training already saturates and the contrastive intervention adds nothing measurable; classical OR baselines also reach near-optimum on these cases. The diagnostic therefore generalises across geospatial planning regimes; the intervention is conditional on the ranking-failure regime that the diagnostic identifies. We release the trained ensembles, an open synthetic benchmark, the public-data restoration cases, and a deployable ArcGIS Pro toolbox alongside a non-commercial CLI workflow.
