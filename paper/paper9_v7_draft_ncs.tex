...
\title{Pairwise ranking restores model-based planning in high-branching discrete action spaces, with application to county-scale farmland consolidation}
...
\subsection*{Aggregate fidelity and within-state action ranking are independent quantities}\label{sec:fidelity-vs-ranking}
...
% ============================================================================
\section{Cross-domain verification on a non-farmland natural-resources case}\label{sec:cross-domain}
% ============================================================================

The synthetic stress tests above vary geometry and fragmentation but keep the underlying decision semantics (paired farmland--forest swaps under township-level swap budgets, area-weighted slope and contiguity rewards) identical to Sections~\ref{sec:counties}. To probe whether the methodological contribution of \S\ref{sec:fidelity-vs-ranking}--\ref{sec:fix} extends beyond farmland consolidation, we apply the unmodified pipeline to a structurally different geospatial planning problem: ecological-restoration prioritisation for abandoned mine lands. The decision unit is a planning cell (not a cadastral block), the action is a one-time selection of a unit for restoration (not a reversible paired swap), and the reward is a linear combination of risk reduction, water-quality protection, spatial connectivity, and a cost penalty (not the farmland multi-objective reward of Equation~\ref{eq:reward}). We test on two cases: a real public-data instance from Buchanan County, Virginia, USA ($n_{\text{units}}=562$ planning cells at 2\,km resolution; $1{,}249$ abandoned-mine point records from the OSMRE e-AMLIS public inventory; $3{,}157$ NHD flowlines for water-priority scoring; USGS 3DEP slope at 30\,m resolution), and a deterministic synthetic instance with $n_{\text{units}}=420$ on a comparable scale. The pipeline (Tools 1--4) is structurally unchanged: only the per-unit feature vector and the multi-objective reward weights are swapped (\S\ref{sec:methods-restoration}).

Three findings emerge (Figure~\ref{fig:restoration}).

\paragraph{Contrastive MPC outperforms a uniform-random selector by orders of magnitude on both cases.} On Buchanan, the random baseline achieves a cumulative episode reward of $+88.7 \pm 12.7$ (5 seeds), while the contrastive 5-ensemble MPC pipeline obtains $+229.4 \pm 0.0$, a $+158\%$ lift over random. On the synthetic instance the corresponding numbers are $+6{,}163 \pm 160$ random vs $+8{,}128 \pm 63$ contrastive, a $+32\%$ lift. The cross-seed variance under the deterministic-given-data planner is essentially zero in both cases, mirroring the reproducibility property already documented for farmland (\S\ref{sec:counties}). The contrastive pipeline thus successfully transfers, as a planning system, to a non-farmland geospatial decision problem at no algorithmic adaptation cost beyond the env-class instantiation.

\paragraph{The aggregate-fidelity vs ranking-accuracy decoupling is regime-dependent: it is acute in farmland but absent in restoration.} Training the same architecture under MSE-only ($\lambda_{\text{rank}}=0$) and contrastive ($\lambda_{\text{rank}}=5$) objectives, we measure pairwise ranking accuracy on each case's held-out pairwise dataset (Figure~\ref{fig:restoration}b). On both real and synthetic restoration cases, MSE-only training already reaches $\geq 97\%$ pairwise accuracy, indistinguishable within noise from the contrastive variant ($\geq 98\%$). On Buchanan, MPC end-task reward at $\lambda{=}0$ ($+229.3$) is statistically identical to $\lambda{=}5$ ($+229.4 \pm 0.0$); on synthetic, the gap is $+8{,}245$ vs $+8{,}128 \pm 63$, also within noise. This is in stark contrast to the farmland regime, where the MSE-only model achieves only $51.6\%$ pairwise accuracy ($60.2\%$ even after a fresh re-train; \S\ref{sec:rank}, Table~\ref{tab:contrastive-disc}) and the corresponding MPC variant collapses entirely (greedy continuation: $+0.029\%$ slope, wrong direction).

\paragraph{The diagnostic of \S\ref{sec:fidelity-vs-ranking} predicts where ranking failure should appear, and the prediction is now confirmed in both directions.} The pathological example of \S\ref{sec:fidelity-vs-ranking}---the conditional-mean predictor minimising MSE while giving 50\% rank accuracy---requires that the within-state variance of the reward across actions be small relative to the between-state variance. In farmland consolidation the per-action reward is a difference of geometric quantities (slope deltas, contiguity deltas, qualifying-area deltas) computed under a single block selection; with five paired swaps per step on a typical 2{,}600-block county, per-action true-reward standard deviation across the action menu is $0.811$ on Bishan---small enough that an MSE-trained predictor can collapse the action distribution to the conditional mean and still attain near-optimal aggregate loss. In restoration, the per-action reward is read directly from the per-unit attribute table (risk index, water priority, spatial connectivity contribution); per-action true-reward standard deviation across action menus is $0.47$ on Buchanan and $34.4$ on synthetic, both large in relation to between-state variance because the action effectively reads a per-unit lookup. MSE training has no incentive to compress this signal, and indeed it does not. The contrastive intervention is therefore not universally needed; it is needed precisely where the reward landscape is dominated by between-state variance with small within-state variance, of which farmland consolidation is a representative case.

This negative result on restoration is informative rather than disappointing. It tightens the methodological claim from "MSE-trained learned models always under-rank actions" to "MSE-trained learned models can collapse to the conditional-mean predictor (and therefore under-rank actions) in regimes where within-state action variance is small relative to between-state variance"---a more falsifiable and more usable prediction for practitioners deciding whether to add the auxiliary loss to their own pipeline.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{restoration_verification.pdf}
\caption{Cross-domain verification on natural-resources restoration cases. (a)~Total episode reward (normalised per case to expose relative ordering on a single axis) for random-uniform selection, MSE-only ($\lambda_{\text{rank}}{=}0$) MPC, and contrastive ($\lambda_{\text{rank}}{=}5$) MPC, on the Buchanan VA real-data case (562 units, $1{,}249$ AML points, USGS 3DEP slope) and the synthetic mine-restoration case (420 units, parameter-controlled). MPC outperforms random by $+158\%$ on Buchanan and $+32\%$ on synthetic; the contrastive intervention provides no measurable additional benefit in either case. (b)~Pairwise ranking accuracy by domain and training objective. Farmland (Bishan, Neijiang) shows the large MSE-only-vs-contrastive gap that motivates the paper's auxiliary ranking loss; restoration shows MSE-only already saturating at $\geq 97\%$, so the contrastive term has no headroom to improve. (c)~Reward-component decomposition on Buchanan: contrastive and MSE-only allocate cumulative reward across the four objective terms identically within noise, confirming that the two policies are essentially equivalent on this case. The contrastive intervention is therefore regime-specific rather than universal, exactly as the within-state-variance argument of \S\ref{sec:fidelity-vs-ranking} predicts.}
...
For the Buchanan VA case ($n_{\text{units}}=562$), per-unit features are derived from the OSMRE e-AMLIS national abandoned-mine-lands inventory \citep{osmre2019eamlis}, USGS NHD flowline data \citep{usgs2024nhd}, USGS 3DEP elevation \citep{usgs2024_3dep}, and Census TIGER county boundaries \citep{census2023tiger}, all freely available as public open data; episode length is 50 steps and the cumulative cost cap is \$200{,}000 (notional units in the e-AMLIS unfunded-cost field). For the synthetic case ($n_{\text{units}}=420$), feature, adjacency, and reward generators are deterministic given a seed and are released alongside the synthetic benchmark of \S\ref{sec:bench}; episode length is 60 steps and the cost cap is \$45{,}435. Both cases share the same MPC configuration as farmland ($H{=}5$, $K{=}50$, greedy continuation, scoring=reward) and the same training hyperparameters ($\lambda_{\text{rank}}{=}5.0$ for contrastive, margin $0.1$, three-member ensembles per seed, five seeds for the contrastive condition, one ensemble for the MSE-only baseline).
