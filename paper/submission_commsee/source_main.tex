...
\begin{abstract}
% ============================================================================
Farmland consolidation is a major spatial-planning lever for food security and erosion control, yet county-scale optimisation has remained out of reach for routine planning offices: the cadastral action space is high-branching, rewards are multi-objective, and reinforcement-learning baselines on this task require $8$--$12$ GPU-hours per seed. Here we show that a learned transition-model and model-predictive-control pipeline, trained in $25$\,min and planning each scenario in $\sim$$7$\,min on a desktop CPU, reduces area-weighted farmland slope by $-1.289 \pm 0.079\%$ on Bishan District ($52{,}515$ parcels) and reproduces on Neijiang Dongxing ($76{,}376$ parcels), $1.6\times$ the reduction reached by centralised-PPO and multi-agent baselines. An auxiliary pairwise ranking loss resolves a hidden failure of the mean-squared-error objective when within-state reward variance is small relative to between-state variance, the regime that farmland inhabits. A public-data US restoration cross-domain case and a simulator-cost crossover analysis bound when the pipeline beats classical operations-research baselines. The full pipeline ships as an ArcGIS Pro toolbox and a non-commercial CLI.
\end{abstract}
...
The clearest test of the intervention is whether it produces planning outcomes that a county consolidation officer would recognise as useful, on the actual cadastral data the officer would supply. We deployed the contrastive-MPC pipeline end-to-end on Bishan District, Chongqing (52{,}515 parcels, 2{,}600 blocks), evaluating five independently trained ensembles for 100 planning steps each (one episode per ensemble). The headline behaviour is summarised in Figure~\ref{fig:bishan-headline}: panel (a) shows the per-step slope-reduction trajectory across all five ensembles---tightly clustered, $\sigma{=}\pm0.001$\,pp at step 100---against the in-house Centralised PPO and MARL baselines under matched objectives; panel (b) shows the spatial distribution of the 852 paired farmland--forest swaps that the planner commits on the optimised cadastre. The outcomes are reported per seed in Table~\ref{tab:contrastive-multiobj} and aggregated as cross-seed mean $\pm$ standard deviation.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{bishan_headline.pdf}
\caption{Bishan headline result. (\textbf{a}) Per-step slope-reduction trajectory across five evaluation episodes of the shipped Bishan ensemble (regime B/C of \S\ref{sec:counties}; mean trajectory in bold blue, 5-episode envelope shaded). The ensemble is deterministic across the action-selection path under fixed seed, so the five episodes overlap to within $0.002$\,pp at every step. Both endpoint annotations sit beneath the in-house Centralised PPO ($-0.79 \pm 0.36\%$, dashed) and Multi-Agent RL ($-0.81 \pm 0.10\%$, dotted) baselines; the lab-pipeline regime-A result of $-1.289 \pm 0.079\%$ (Table~\ref{tab:contrastive-multiobj}) sits in between. The endpoint-stack on the right reports the env-side slope ($-2.04\%$, mean of 5 episodes) and the independent GIS-recompute verification ($-2.001\%$, Methods \S\ref{sec:methods-validate}) that anchors the regime-C reading. (\textbf{b}) Spatial distribution of the 852 paired swaps committed by the planner on the optimised cadastre (episode 0): 426 farm$\to$forest parcels (red, slope-shed) and 426 forest$\to$farm parcels (green, slope-take), buffered to $250$\,m for visibility at county scale. Swaps concentrate at fragment boundaries where slope-reducing trades cost the least qualifying area, the morphological signature behind the trade-off discussed in the next paragraph.}
\label{fig:bishan-headline}
\end{figure}

All three policy-relevant objectives move in the intended direction simultaneously. Area-weighted mean farmland slope decreases by $1.289\% \pm 0.079\%$ relative to the initial $9.62^{\circ}$ (CV $\approx 6\%$ across seeds), within-farmland contiguity rises by $0.0160 \pm 0.0016$, and the count of qualifying \textit{baimu fang} ($\geq 6.67$\,ha contiguous farmland units) increases by $+3.4 \pm 1.0$ relative to the initial 109 patches (Bishan baseline; per-seed values in Supplementary Table~\ref{tab:si-contrastive-multiobj}). One axis moves against the planner: total \textit{baimu fang} area decreases by $-312 \pm 34$\,ha, equivalent to $0.67\%$ of the initial $46{,}844$\,ha qualifying stock. This trade-off is intentional rather than incidental. Slope-reducing swaps disproportionately retire marginal farmland from the largest existing patches, while the count and contiguity reward terms pull the remaining area toward more compact configurations. The net planning effect---more qualifying large patches, lower total qualifying area, higher contiguity---matches the morphological intent of Chinese consolidation policy, where the regulatory constraint on absolute farmland area is enforced by a separate ``red line'' mechanism that our paired farmland--forest swap respects by construction (Methods, \S\ref{sec:methods-action}).

The intervention is competitive with model-free reinforcement-learning baselines we ran on the same task (Centralised PPO and a multi-agent variant; Supplementary Material \S\ref{si:s1-drl}). Slope improvement under contrastive MPC ($-1.289 \pm 0.079\%$) is approximately $1.6\times$ larger than the multi-agent baseline ($-0.81 \pm 0.10\%$; Welch's $t = -8.01$, $\mathrm{df}{=}7.9$, $p < 10^{-4}$, Cohen's $d = -5.07$) and the Centralised PPO baseline ($-0.79 \pm 0.36\%$; Welch's $t = -3.01$, $\mathrm{df}{=}4.5$, $p = 0.034$, Cohen's $d = -1.90$), with comparable contiguity ($+0.016$) and \textit{baimu fang} count ($+3.4$ versus $+3.2$ and $+2.9$). Both effect sizes exceed the conventional ``very large'' threshold ($|d| > 1.2$). Cross-seed slope variance is $4.6\times$ smaller than Centralised PPO and similar to MARL's, indicating consistent rather than lucky behaviour. The total qualifying-area loss is larger ($-312$\,ha versus $-75$ to $-97$\,ha), an honestly reported trade-off discussed in \S\ref{sec:discussion}. The cost asymmetry is the substantive deployment headline: the contrastive ensemble trains in 25 minutes on a commodity 12-thread CPU and runs each 100-step planning episode in approximately 225 seconds, against $8$--$12$ GPU-hours per seed for the in-house DRL baselines---an end-to-end wall-clock difference of roughly two orders of magnitude.

We also evaluated classical operations-research baselines on Bishan at a 180\,s wall budget matched to the contrastive-MPC pipeline: simulated annealing over a permutation of block selections reached cumulative episode reward $9.04$ at $100$ iterations, random-restart over $98$ episodes reached $30.85$, and contrastive MPC reached $86.74$---a $9.6\times$ gap over simulated annealing. The 180-s comparison is reported as a single matched-budget run per algorithm rather than as a seeded distribution because the SA and random-restart baselines are deterministic-by-construction at fixed RNG and therefore degenerate as a between-seeds comparison; for the contrastive-MPC pipeline, the cross-seed reward standard deviation in the open package-side reproduction (5 ensembles, Supplementary \S\ref{si:bishan-regimes}) is $\sigma(R) = 7.4$ at the comparable operating point, an order of magnitude below the headline gap. The mechanism (the simulator's $17.9$\,ms/step cost combined with combinatorial cost interactions in the reward function) is developed in \S\ref{sec:simcost}.
...
\paragraph{Three Bishan replication regimes.} The Bishan slope numbers cited in this and subsequent sections are reported across three internally-consistent regimes that differ along two protocol axes and one data-pipeline axis. \emph{Regime A} (the canonical headline of this section, $-1.289 \pm 0.079\%$) is 5 lab-trained ensembles $\times$ 1 episode each on the research-side prepare (2{,}600 blocks, 52{,}515 parcels). \emph{Regime B} (the toolchain replication, \S\ref{sec:deploy}, $-1.544 \pm 0.041\%$) is 1 shipped ONNX ensemble $\times$ 5 episodes on the package-side prepare (2{,}640 blocks, 53{,}004 parcels), whose corrected geographic-CRS slope step (Methods, \S\ref{sec:methods-prepare}) yields modestly more reduction headroom on the same county. \emph{Regime C} (the verification anchor of the next paragraph and Methods, \S\ref{sec:methods-validate}, $-2.001\%$) is 1 shipped ensemble $\times$ 1 canonical episode on the package-side prepare, evaluated by an independent GIS-only recomputation script that retains 82 extra township-registry-external parcels with \texttt{CHG\_FLAG}=0 (53{,}086 parcels total; the additional parcels do not contribute to any delta). Across regimes the contrastive-MPC slope improvement is consistently in the $-1.3$ to $-2.0$ pp band; we report all three at face value rather than reduce them to a single number because the gap is itself informative about deployment variance, and a complete cross-regime reconciliation table is reported in Supplementary Material \S\ref{si:bishan-regimes}.

\paragraph{Robustness.} Four independent verification layers underwrite the Bishan and Neijiang results. (i)~A direct GIS recomputation from the optimised cadastral shapefile written to disk reproduces the slope, contiguity, and \textit{baimu fang} deltas to floating-point noise without invoking the learned ensemble (\S\ref{sec:methods-validate}, regime C); the headline gain is therefore a property of the on-disk geometry, not of the learned dynamics. (ii)~The contrastive ensemble's $1$-step prediction error is measured directly against the true environment along the same $100$-step trajectory MPC visits (\S\ref{sec:methods-mae}); the ensemble is mis-calibrated as a regressor (reward MAE $0.82$ against $\sigma(r_{\text{true}})=1.40$) yet recovers a Spearman rank correlation of $+0.22$ sufficient at $H{=}5$ to reproduce the action sequence to floating-point identity, empirically confirming the ranking-versus-regression decomposition. (iii)~A counter-factual ablation that replaces the ensemble entirely with the true \texttt{env.step} inside the scoring loop returns a slope of $-1.819\%$ in $358$\,s, strictly worse on all three objectives than the regime-C ensemble pipeline's $-2.001\%$ in $1{,}448$\,s (\S\ref{sec:methods-trueenv}). (iv)~Cross-county replication on Neijiang shows the qualitative result survives a $1.4\times$ scale-up and a different morphology.
...
\paragraph{Methodological mechanism.} Two mechanisms are stacked behind these gains, both characterised in §\ref{sec:rank} and §\ref{sec:simcost} respectively. The auxiliary pairwise large-margin term closes a hidden ranking failure of the standard MSE objective in the $\sigma_a/\sigma_s \!\lesssim\! 1$ regime farmland inhabits, and is redundant when reward is a per-unit attribute lookup. The learned-surrogate pipeline as a whole, separately, beats classical operations-research baselines when each simulator step is computationally expensive ($\geq 30$\,ms in our measurements) or when reward exhibits combinatorial cost interactions, or both---farmland satisfies both conditions, the empirical reason the pipeline dominates farmland but not restoration default.

\paragraph{Practical implication for problem selection.} Practitioners considering a learned-surrogate planner on a new domain should report two quantities: simulator step cost (in ms/step) and whether reward exhibits combinatorial cost interactions. When step cost exceeds $\sim 30$\,ms or combinatorial structure is non-trivial, the learned-surrogate pipeline is in the advantage regime; otherwise well-tuned classical methods reach near-optimum at lower wall-clock cost. The two-quantity criterion is calibrated on the present farmland and restoration evidence, not benchmarked across other Earth-science domains; we conjecture it extends to applications where high-fidelity physics simulators dominate per-step cost---hydrologic-model-driven watershed restoration, integrated power-grid maintenance scheduling, multi-stage chemical-reaction prioritisation, supply-chain optimisation under economic-equilibrium models---all of which combine non-trivial step cost with combinatorial constraint structure, but those extensions remain testable hypotheses for future work rather than established results.
...
The resulting trajectory finishes at slope $-1.819\%$, contiguity $+0.0087$, and \textit{baimu fang} area change $-565$\,ha, all three strictly worse than the regime-C ensemble pipeline's $-2.001\%$, $+0.0128$, $-473$\,ha. Per-step inspection shows true-env MPC starting behind (the $200$-action stage~1 misses the strongest moves the ensemble identifies via full-action coverage), narrowing the gap as the trajectory matures, and finishing $0.18$\,pp short of the ensemble's slope. The reason is structural rather than statistical: ensemble \texttt{batch\_predict} grants the planner the depth and breadth that incremental simulation cannot afford within the same compute budget. Putting matched effort into true-env MPC---full enumeration in stage~1 and greedy continuation in stage~2---would cost approximately five days of wall time per episode for a payoff of at most a fraction of a percentage point of slope. The headline regime-C $-2.00\%$ result is therefore on the optimistic side of the planner's accessible frontier under matched compute, and the ensemble's contribution to the deployment is structural (it grants the planner depth and breadth) rather than illusory.
